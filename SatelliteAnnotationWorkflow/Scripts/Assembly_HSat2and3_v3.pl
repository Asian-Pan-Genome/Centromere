...
my $usage = "perl Assembly_HSat2and3_v3.pl /path/to/reference.fasta output_directory";
...
if(defined $ARGV[1]){
         ...
         die "no output_directory specified!\n\n default: running directory\n\n";

}
#create output file name
my $outfilename = "";
if($reffile=~/.+\/(.+)\..+?$/){
	$outfilename = "$1.HSat2and3_Regions.bed";
}
else{
	$outfilename = "$reffile.HSat2and3_Regions.bed";
}

my $outfile = File::Spec->catfile($output_dir, $outfilename);
