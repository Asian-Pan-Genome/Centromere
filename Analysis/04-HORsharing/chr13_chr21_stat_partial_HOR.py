...
def extract_position(outprefix, mn_bed, ihor, genome, idx, ihor_flag):
    ...
    with open(mn_bed, 'r') as f:
        for line in f:
            ...
            if mn == shared_sequences[0] and start_shared_index is None:
                start_shared_index = index

            if mn == shared_sequences[-1] and end_shared_index is None:
                end_shared_index = index
        ...
        if len(set(strands)) == 1:
            ...
            ###output shared_sequence####
            if (start_shared_index is not None) and (end_shared_index is not None):

                if start_shared_index < end_shared_index:

                    print("check! shared mns: ",  mns[start_shared_index : end_shared_index +1])

                    shared_original_positions = original_positions[start_shared_index : end_shared_index + 1]
                    with open(f"tmp/{outprefix}_tmp_shared_s.bed", 'w') as out1:
                        out1.write(f"{outprefix}\t{shared_original_positions[0][0]}\t{shared_original_positions[-1][1]}\n")
                        out1.flush()
                        os.fsync(out1.fileno())
                        run_bedtools_getfasta(genome, f"tmp/{outprefix}_tmp_shared_s.bed", f"tmp/{outprefix}_tmp_shared_s.fa")
                else:

                    print("check! shared mns: ", mns[start_shared_index:] + mns[: end_shared_index + 1])

                    shared_reorder_positions_s = original_positions[start_shared_index:]
                    shared_reorder_positions_e = original_positions[: end_shared_index + 1]

                    with open(f"tmp/{outprefix}_tmp_shared_s.bed", 'w') as out1:
                        out1.write(f"{outprefix}\t{shared_reorder_positions_e[0][0]}\t{shared_reorder_positions_e[-1][1]}\n")
                        out1.flush()
                        os.fsync(out1.fileno())
                        run_bedtools_getfasta(genome, f"tmp/{outprefix}_tmp_shared_s.bed", f"tmp/{outprefix}_tmp_shared_s.fa")

                    with open(f"tmp/{outprefix}_tmp_shared_e.bed", 'w') as out2:
                        out2.write(f"{outprefix}\t{shared_reorder_positions_s[0][0]}\t{shared_reorder_positions_s[-1][1]}\n")
                        out2.flush()
                        os.fsync(out2.fileno())
                        run_bedtools_getfasta(genome, f"tmp/{outprefix}_tmp_shared_e.bed", f"tmp/{outprefix}_tmp_shared_e.fa")

            out_shared_seq = concat_fasta_sequences(f"tmp/{outprefix}_tmp_shared_s.fa", f"tmp/{outprefix}_tmp_shared_e.fa")
            ...
            if os.path.exists(f"tmp/{outprefix}_tmp_shared_s.bed"):
                os.remove(f"tmp/{outprefix}_tmp_shared_s.bed")
            if os.path.exists(f"tmp/{outprefix}_tmp_shared_e.bed"):
                os.remove(f"tmp/{outprefix}_tmp_shared_e.bed")
            if os.path.exists(f"tmp/{outprefix}_tmp_shared_s.fa"):
                os.remove(f"tmp/{outprefix}_tmp_shared_s.fa")
            if os.path.exists(f"tmp/{outprefix}_tmp_shared_e.fa"):
                os.remove(f"tmp/{outprefix}_tmp_shared_e.fa")
        else:
...
def stat_target_hor(horstv_bed, mnbed, genome, outprefix):
    ...
    horstv = horstv[(horstv['chrom'] == r_chrom) & (horstv['horclass'] == '42')].copy()
    ...
    target_hor = {
    "2975_24578_3036_25494_3059_24578_3072_24569_3042_25511_3379": "h1_11mer-24578", 
    "2975_24578_3036_25494_3059_24579_3072_24569_3042_25511_3379" : "h2_11mer-24579", 
    "2975_24578_3036_25494_3379_3072_24569_3042_25511_3379" : "h3_10mer", 
    "2975_24578_3072_24569_3042_25511_3379"  :  "h4_7-mer-24578", 
    "2975_24579_3072_24569_3042_25511_3379"  : "h5_7mer-24579" 
    }
