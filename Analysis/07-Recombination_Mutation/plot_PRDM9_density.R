...
setwd("/share/home/zhanglab/user/sunyanqing/human/periphy/CHM13/recombination")
...
if(sample == "CHM13"){
  ...
  for (target_chrom in unique(all_stat_df$chrom)) {
    if (grepl("chr", target_chrom) && !grepl("chrY-CEN", target_chrom)){
      plot_df <- all_stat_df %>% filter(chrom == target_chrom)
      ...
      geom_rect(cent_regions, mapping = aes(xmin = start, xmax = end, ymin = 950, ymax = 1000,  fill = color), inherit.aes = FALSE) +
      scale_fill_identity() + 
      new_scale("fill") +  
      geom_rect(horstv_regions, mapping = aes(xmin = start, xmax = end, ymin = 850, ymax = 900, fill = horstv_newcolor), inherit.aes = FALSE) +
      scale_fill_identity() + 
      # facet_wrap(~motif, ncol = 1) +
      theme_minimal() +
      theme(
    ...
    print(p)
    ggsave(p, filename = paste0(sample, "_", hap, "/", "PRDM9_density_", target_chrom, ".png"), width = 6, height = 3, dpi=600)
    }
  }
}
